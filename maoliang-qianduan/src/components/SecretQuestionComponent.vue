<template>
  ...
  <div class="login">
    <form @submit.prevent="submitSecretAnswer">
      <div class="input">
        <h3>用户{{ username }}，请您回答密保问题</h3>
        <h4>{{ secretQuestion }}</h4>
...
export default {
  data() {
    return {
      username: 'userExample', // 从路由参数或其他途径获取
      secretQuestion: 'who are you?', // 从后端获取或其他途径
      secretAnswer: 'seller',
      error: null,
    };
  },
  methods: {
    async submitSecretAnswer() {
      ...
      try {
        alert("您的密码是：123");
        this.$router.push('/');
        // const response = await axios.post('/api/secret-answer', { username: this.username, answer: this.secretAnswer });
        // 处理响应
        // this.$router.push({ name: 'Success', params: { message: '密保答案正确', to: '/' } });
      } catch (err) {
