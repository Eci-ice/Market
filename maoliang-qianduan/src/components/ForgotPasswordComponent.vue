  ...
  methods: {
    async submitForgetPassword() {
      ...
      try {
        ...
        // const response = await axios.post('/api/forget', { username: this.forName });
        
        // 假设您已经成功获取了密保问题
        // const secretQuestion = response.data.secretQuestion;
        
        // 跳转到密保问题回答页面，并传递用户名和密保问题
        this.$router.push({ 
          name: 'SecretQuestion', 
          params: { username: this.forName /*, secretQuestion: secretQuestion*/ }
        });
      } catch (err) {
        this.error = '找回密码时出错';
      }
