<template>
  <ul class="daohang">
    <li><h2>"喵咪美食坊"欢迎您! This is test server</h2></li>
    ...
    async handleLogin() {
      try {
        ...
        if (responseData.page === "error") {
          ...
          this.$router.push({
            path: "/error",
            query: { err: responseData.msg, to: responseData.data },
          });
        } else if (responseData.page === "success") {
          // 重定向到成功界面，并将成功消息和重定向目标作为参数传递
          this.$router.push({
            path: "/success",
            query: { message: responseData.msg, to: responseData.data },
          });
        } else if (responseData.page === "buyer") {
          // 处理其他情况，例如重定向等
          // 重定向到买家主界面
